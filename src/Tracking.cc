...
//zoe 20181016
Tracking::Tracking(System *pSys, LFNET* pLFNet, LFNETVocabulary* pVocLFNet, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Map *pMap, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor):
    mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(false), mbVO(false), mpLFNet(pLFNet), mpLFNETVocabulary(pVocLFNet),
    mpKeyFrameDB(pKFDB), mpInitializer(static_cast<Initializer*>(NULL)), mpSystem(pSys), mpViewer(NULL),
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    // Load camera parameters from settings file
...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp)
{
    ...
    //zoe 20181016
    mCurrentFrame = Frame(mImGray,imDepth,timestamp,mpLFNet,mpLFNETVocabulary,mK,mDistCoef,mbf,mThDepth);
...
void Tracking::MonocularInitialization()
{
    ...
    {
        // Set Reference Frame
        if(mCurrentFrame.mvKeys.size()>100)//zoe nochange
        {
            mInitialFrame = Frame(mCurrentFrame);
            mLastFrame = Frame(mCurrentFrame);
            mvbPrevMatched.resize(mCurrentFrame.mvKeysUn.size());//zoe nochange
            for(size_t i=0; i<mCurrentFrame.mvKeysUn.size(); i++)//zoe nochange
                mvbPrevMatched[i]=mCurrentFrame.mvKeysUn[i].pt;//zoe nochange
    ...
    {
        // Try to initialize
        if((int)mCurrentFrame.mvKeys.size()<=100)//zoe nochange
        {
            delete mpInitializer;
...
{
    for(int i =0; i<mLastFrame.N; i++)//zoe N也要改
    {
...
bool Tracking::TrackLocalMap()
{
    ...
    if(mnMatchesInliers<30)// zoe 20181021 修改
        return false;
    else
        return true;
}
