...
Frame::Frame(const Frame &frame)
    :mpLFNETvocabulary(frame.mpLFNETvocabulary),mpORBvocabulary(frame.mpORBvocabulary), mpLFNet(frame.mpLFNet),mpORBextractorLeft(frame.mpORBextractorLeft), mpORBextractorRight(frame.mpORBextractorRight),//zoe 20181016
...
//zoe 20181016
/* //该程序是离线读取照片特征点和描述子的程序,暂时弃用,20181025
     ...
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
    ...
    string strLFNetPath = "/home/zoe/data/rgbd_dataset_freiburg1_room/LFNet/";
...
*/
// zoe 20181025
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, LFNET* extractor, LFNETVocabulary* voclfnet, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth)
    :mpLFNETvocabulary(voclfnet),mpLFNet(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth)
{
    // Frame ID
    mnId=nNextId++;
    ...
    mvDspts.clear();
    mvKpts.clear();
    mpLFNet->Predict(imGray);
    mvKpts = mpLFNet->mvkpts;
    mvDspts = mpLFNet->mvdspts;
