        ...
        mbDeactivateLocalizationMode(false)
{
    ...
    bool bVocLoadORB = mpVocabularyLFNet->loadFromTextFile(strVocFile);
    if(!bVocLoadORB)
    {
    ...
    mpKeyFrameDatabase = new KeyFrameDatabase(*mpVocabulary);// zoe database名字没改
...
// zoe 20181025
System::System(const string &strVocFile, const string &strSettingsFile,const string &strLFNetFile, const eSensor sensor,
               const bool bUseViewer):mSensor(sensor), mpViewer(static_cast<Viewer*>(NULL)), mbReset(false),mbActivateLocalizationMode(false),
        mbDeactivateLocalizationMode(false)
{
    // Output welcome message
    cout << "This is LF-SLAM, based on ORB-SLAM2. Now only support for RGB-D sensor!" << endl;
    ...
    //zoe 20181025 
    int input_width = fsSettings["Camera.width"];
    int input_height = fsSettings["Camera.height"];   
    int kpt_k = fsSettings["ORBextractor.nFeatures"];
    int dspt_k = 256;
    // zoe 20181025 lfnet 创建类不能放在加载字典前面,不然的话总是创建不成功,不知道为什么
    mpLFNet = new LFNET(strLFNetFile.c_str(), input_width, input_height, kpt_k, dspt_k);
    // zoe 等待模型加载完毕,不然报错
    while(mpLFNet->mload_model==0)
    {
        usleep(1000);
    }
    ...
    //========================================================//
    //zoe 20181016 track名字没改
    mpTracker = new Tracking(this, mpLFNet, mpVocabularyLFNet, mpFrameDrawer, mpMapDrawer,
                             mpMap, mpKeyFrameDatabase, strSettingsFile, mSensor);
    //========================================================//
    //Initialize the Local Mapping thread and launch
    mpLocalMapper = new LocalMapping(mpMap, mSensor==MONOCULAR);
